# EnviroCare Brand Knowledge Base

## Company Identity
- **Founded**: 1958 by Phillip M. Wedgworth in Alexander City, Alabama
- **Current owner**: Kevin Wedgworth (3rd generation)
- **Structure**: Family-owned, never franchised, never sold
- **Tagline**: "No One Cares Like EnviroCare"
- **Logo**: Green circle with white sunflower

## Three Offices
| Office | Address | Phone | Market |
|--------|---------|-------|--------|
| Birmingham/Alabaster | 2025 Butler Rd, Alabaster AL 35007 | (205) 940-6360 | Greater Birmingham |
| Lake Martin/Alex City | 1785 Tallapoosa St, Alexander City AL 35010 | (256) 234-6162 | Lake Martin, Auburn, East AL |
| Huntsville | 7027 Old Madison Pike Ste 108, Huntsville AL 35806 | (256) 937-7676 | North Alabama |
| Main line | — | (205) 940-6360 | All markets |

## Core Services & Pricing (locked model — June 2026)
RULE: termite is NEVER a flat number to customers — always "quoted after a free WDO inspection."
Mosquito/tick without a pest plan are PER-VISIT only (no monthly). Tick rides with mosquito only.
- **Pest only**: From $35/mo (bimonthly) · $79 startup · ACH · **12-month agreement, equal averaged payments**; per-visit $70 is contract-free
- **Pest + Mosquito**: From $69/mo · $99 startup · add tick +$20/visit — LEAD/most popular
- **Complete (Pest + Termite + Mosquito)**: From ~$100/mo · $229 startup · termite priced upon inspection & approval · add tick +$20/visit — best value
- **Mosquito**: $45/visit × 9 (Mar–Nov) alone; $34/mo ONLY when bundled with a pest plan (monthly needs a year-round anchor)
- **Mosquito + Tick**: $65/visit × 9 (Mar–Nov) — tick rides with mosquito only, never standalone
- **Termite (Sentricon®)**: NO flat price — quoted after a free on-site WDO inspection (Alabama requires it). Up to $1M repair coverage. (Internal only: $325 install incl. year-1 + $380/yr renewal — never quote as a monthly.)
- **Fire ant**: $150 minimum, priced by sq ft — standalone, available to anyone (NOT in the pest plan)
- **WDO (real estate termite letter)**: $75 standalone; one free/yr for active customers

## Strongest Differentiators (in order of impact)
1. **Sentricon® Certified Specialist** — up to $1,000,000 in damage repair coverage, subject to the terms of the agreement. This is EnviroCare's OWN coverage, NOT Corteva's warranty — never attribute it to the manufacturer, and never call it a "guarantee" or "warranty".
2. **68-year track record** — founded 1958, predates most competitors
3. **Four generations of ownership** — same family, same values. NEVER write "third generation".
4. **Lake Martin exclusivity** — ONLY pest control company with a dedicated Lake Martin office
5. **Two ways to pay** — $35/mo on a 12-month ACH billing agreement in equal averaged payments, or $70 per bi-monthly visit. NEVER claim "no contract", "no long-term contract", "no agreement" or "cancel anytime": agreement practice is not uniform and some per-visit customers sign one. Say terms are confirmed in writing before service starts.
6. **Local humans answer the phone** — no call center routing
7. **Same technician every visit** — customers know who is coming
8. **Fast local scheduling** — local routing and four Alabama offices mean quick turnaround, often within 24–48 hours (never promise same-day)

## Customer Profile
- Alabama homeowners, age 35–65
- Value local family businesses over national chains
- Trust built through familiarity and multi-year relationships
- Lake Martin customers: seasonal/vacation home owners, higher income, premium service expectations
- Huntsville customers: military/NASA families, new construction buyers
- Birmingham customers: established neighborhoods with older housing stock (termite risk)

## Brand Voice
- Warm, direct, Southern
- Never corporate-sounding
- Specific over vague ("68 years" not "decades of experience")
- Local references (Alabama weather, clay soil, lake country)
- Honest about what works and what doesn't (e.g., bug zappers don't work for mosquitoes)

## Website
- **Production domain (post-DNS-flip)**: https://envirocarellc.com
- **GitHub repo**: envirocare-website
- **Vercel project**: envirocare-web (single project; deploys every push from the repo)
- **Preview deployments**: per-branch URLs under envirocare-50d39ae8.vercel.app, surfaced on the corresponding GitHub PR
