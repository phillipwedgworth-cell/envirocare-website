...
export const BANNED_PATTERNS: BannedTerm[] = [
  ...
  // Billing canon: mosquito changed from per-treatment to monthly on 2026-08-26.
  // These are deliberately price-shaped so legitimate $70 per-visit pest copy passes.
  ...
  { pattern: '\\b(nine|9)[\\s-]+(visit|treatment|round)s?\\b',
    reason: 'superseded nine-treatment mosquito season',
    approvedInstead: '8 treatments March through October' },
  { pattern: '\\btick\\b[^.\\n]{0,100}\\$\\s?20\\b|\\$\\s?20\\b[^.\\n]{0,100}\\btick\\b',
    reason: 'derived standalone tick price published instead of a property quote',
    approvedInstead: 'standalone tick and flea work is quoted for the property; Mosquito + Tick is $65/month' },
  ...
  //       stands behind/guarantees/warrants/covers
  ...
  { pattern: '(coverage|repair|warrant\\w*)[^.]{0,40}\\b(from|by|backed by|through)\\s+(corteva|the\\s+manufacturer|sentricon)|\\b(corteva|manufacturer)([\'’]s)?\\s+(guarantee|warranty|coverage)\\b|\\b(corteva|the\\s+manufacturer)\\b[^.]{0,60}?\\b(backs|back|backed|provides|offers|stands\\s+behind|guarantees|warrants|covers|updated)\\b|\\b(corteva|the\\s+manufacturer)\\b[^.]{0,40}\\b(their|its)\\s+(guarantee|warranty|coverage)\\b', notIf: 'not (a )?corteva|not the manufacturer|never attribut|rather than the manufacturer|EnviroCare[\'’]s own|banned|do not say|NEVER',
